...
import { AdminCommentListResponse, WeeklyAccessResponse } from "./types";
...
export async function fetchAdminComments(page: number = 1): Promise<AdminCommentListResponse> {
    const query = new URLSearchParams({ page: String(page) });
    const res = await fetch(`${baseUrl}/comments?${query.toString()}`, { credentials: "include" });
    if (!res.ok) throw new Error(`取得に失敗しました: ${res.status}`);
    return res.json();
}
