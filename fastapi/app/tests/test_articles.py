...
class TestCreateArticle:
    def test_未認証は401(self, no_csrf_client):
        res = no_csrf_client.post("/api/articles", json=VALID_ARTICLE_BODY)

        assert res.status_code == 401

    def test_CSRFトークンなしは403(self, auth_client):
        res = auth_client.post("/api/articles", json=VALID_ARTICLE_BODY)

        assert res.status_code == 403

    def test_CSRFトークン不一致は403(self, auth_client):
        auth_client.get("/api/csrf-token")

        res = auth_client.post(
            "/api/articles",
            json=VALID_ARTICLE_BODY,
            headers={"x-csrf-token": "invalid-token"},
        )

        assert res.status_code == 403

    def test_認証済で記事を作成できる(self, no_csrf_auth_client):
        res = no_csrf_auth_client.post("/api/articles", json=VALID_ARTICLE_BODY)
    ...
    def test_作成した記事が一覧に表示される(self, no_csrf_auth_client):
        no_csrf_auth_client.post("/api/articles", json=VALID_ARTICLE_BODY)

        res = no_csrf_auth_client.get("/api/articles")
...
class TestUpdateArticle:
    ...
    def test_未認証は401(self, no_csrf_client, db):
        article = make_article(db)

        res = no_csrf_client.put(f"/api/articles/{article.id}", json=self.UPDATE_BODY)

        assert res.status_code == 401

    def test_CSRFトークンなしは403(self, auth_client, db):
        article = make_article(db)
    ...
    def test_CSRFトークン不一致は403(self, auth_client, db):
        article = make_article(db)
        auth_client.get("/api/csrf-token")
    ...
    def test_認証済で記事を更新できる(self, no_csrf_auth_client, db):
        article = make_article(db, title="元のタイトル")

        res = no_csrf_auth_client.put(f"/api/articles/{article.id}", json=self.UPDATE_BODY)

        assert res.status_code == 200
        assert res.json()["title"] == "更新タイトル"

    def test_存在しないIDは404(self, no_csrf_auth_client):
        res = no_csrf_auth_client.put("/api/articles/non-existent-id", json=self.UPDATE_BODY)
...
class TestDeleteArticle:
    def test_未認証は401(self, no_csrf_client, db):
        ...
        res = no_csrf_client.delete(f"/api/articles/{article.id}")
    ...
    def test_認証済で記事を削除できる(self, no_csrf_auth_client, db):
        article = make_article(db)

        res = no_csrf_auth_client.delete(f"/api/articles/{article.id}")

        assert res.status_code == 204

    def test_削除後は取得できない(self, no_csrf_auth_client, db):
        article = make_article(db)
        no_csrf_auth_client.delete(f"/api/articles/{article.id}")

        res = no_csrf_auth_client.get(f"/api/articles/{article.id}")

        assert res.status_code == 404

    def test_存在しないIDは404(self, no_csrf_auth_client):
        res = no_csrf_auth_client.delete("/api/articles/non-existent-id")
