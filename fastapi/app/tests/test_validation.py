...
class TestArticleInputValidation:
    def test_タイトルが15文字を超えると422(self, no_csrf_auth_client):
        body = {**VALID_BODY, "title": "あ" * 16}

        res = no_csrf_auth_client.post("/api/articles", json=body)

        assert res.status_code == 422

    def test_概要が100文字を超えると422(self, no_csrf_auth_client):
    ...
    def test_mainImageUrlがhttpでないと422(self, no_csrf_auth_client):
    ...
    def test_mainImageUrlがdataスキームだと422(self, no_csrf_auth_client):
    ...
    def test_mainImageUrlがhttpsなら通る(self, no_csrf_auth_client):
        res = no_csrf_auth_client.post("/api/articles", json=VALID_BODY)

        assert res.status_code == 200

    def test_mainImageUrlがhttpでも通る(self, no_csrf_auth_client):
    ...
    def test_タイトルが空だと422(self, no_csrf_auth_client):
    ...
    def test_本文が空だと422(self, no_csrf_auth_client):
